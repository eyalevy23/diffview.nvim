...
local function thread_label(thread)
  local icon = thread.status == "resolved" and "○"
    ...
    or store.awaiting_human(thread) and (require("diffview.config").get_config().comments.icons or {}).ai
    or "●"
...
function M.pick(opts)
  ...
  if not adapter then
    ...
    require("diffview.navigate").when_files_ready(function()
      require("diffview").open_for_picker(opts.open)
    end, function() M.pick({ awaiting = opts.awaiting }) end)
    return
  end
  ...
  -- Threads awaiting your reply lead, ahead of the ones awaiting the AI.
  local status_rank = { open = 2, applied = 3, resolved = 4 }
  local function rank(t) return store.awaiting_human(t) and 1 or status_rank[t.status] or 9 end
  ...
  if opts and opts.awaiting then
    threads = vim.tbl_filter(store.awaiting_human, threads)
    if #threads == 0 then
      utils.info("[diffview] No threads awaiting you.")
      return
    end
  end
