...
---Place every visible thread for a buffer: match by path, resolve by content.
---Anchors are content-addressed (snippet + context), so a thread renders in
---any view of its file — not only the rev it was created against. The rev is
---still consulted for one thing: only an on-rev view may stamp a thread
---outdated, so browsing history never poisons the store.
---@param bufnr integer
---@return PlacedThread[]
...
function M.comment_apply()
  ...
  -- Applying writes to buf_b, so it must be the real working-tree file — in a
  -- commit range view (e.g. main...HEAD) it's a read-only commit buffer.
  if thread.anchor.side ~= "b" then
    utils.warn("[diffview] Can only apply suggestions anchored to the new side.")
    return
  end
  if ctx.rev_b ~= "WORKING" then
    utils.warn("[diffview] Suggestions can only be applied in a working-tree view (:DiffviewOpen).")
    return
  end
