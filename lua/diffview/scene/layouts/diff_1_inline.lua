  ...
  for i = #hunks, 1, -1 do
    ...
    for oi = old_start, old_start + old_count - 1 do
      if oi >= 1 and oi <= #old_lines then
        old_toks[oi] = inline_hl.tokenize(old_lines[oi])
      end
    end
    for ni = new_start, new_start + new_count - 1 do
      if ni >= 1 and ni <= #new_lines then
        new_toks[ni] = inline_hl.tokenize(new_lines[ni])
      end
    end
    ...
    -- absolute line index.
    local pairs_list = inline_hl.pair_lines(old_toks, new_toks, old_lines, new_lines,
    ...
    if old_count > 0 then
      ...
      local ts_spans_by_lnum = inline_hl.ts_spans_for_lines(
        buf_a,
